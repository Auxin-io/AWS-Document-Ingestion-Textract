...
#
# Run from the repository root:   bash ingest/run_all.sh
#
# Prerequisite: the buckets and ACL table must exist. Deploy them first:
...
python ingest/generate_pdfs.py --out-dir data/pdfs --count "$COUNT_TRAIN" --split train
python ingest/generate_pdfs.py --out-dir data/pdfs --count "$COUNT_TEST"  --split test

echo "== 2/5 upload to the raw bucket =="
python ingest/document_pipeline.py upload data/pdfs/train --prefix incoming/train
python ingest/document_pipeline.py upload data/pdfs/test  --prefix incoming/test

echo "== 3/5 OCR with Textract (~USD 1.50 per 1000 pages) =="
python ingest/document_pipeline.py extract --workers "$WORKERS"
...
# grants the demo users a curated subset instead.
if [ -n "${PRINCIPAL:-}" ]; then
  python ingest/document_pipeline.py register --principal "$PRINCIPAL" --permission "${PERMISSION:-read}"
else
  echo "   (skipped - set PRINCIPAL=<cognito sub> to grant all documents to one user)"
fi

echo "== 5/5 build JSONL from the OCR text =="
python ingest/build_dataset.py --out-dir data/dataset_pdf --max-train "$MAX_TRAIN"
...
Next (in the main project):
  python src/training/start_sagemaker_training.py --dataset-dir data/dataset_pdf
NOTE
